...
namespace InvestmentPlanAPI.Repository
{
    public class InvestorRepository : IInvestorRepo
    {
        ...
        {
            var investor1 = await _context
                .Investors.Include(i => i.InvestorDetail)
                .Include(e => e.Roles)
        ...
        {
            return await _context.InvestorDetails.FirstOrDefaultAsync(d =>
                d.PublicInvestorId == publicInvestorId
            );
        }

        public async Task<IEnumerable<Investor>> GetAllInvestorDetailsAsync()
        {
            return await _context.Investors.Include(e => e.InvestorDetail).ToListAsync();
        }
        ...
        public async Task<InvestorDetails?> FindIfDetailsExist(
            string aadhaarNo,
            string panNo,
            string mobile
        )
        {
            return await _context.InvestorDetails.FirstOrDefaultAsync(x =>
                x.AadhaarNo == aadhaarNo || x.PanNo == panNo || x.Mobile == mobile
            );
        }
