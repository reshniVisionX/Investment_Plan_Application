...
namespace InvestmentPlanAPI.Service
{
    public class TransactionService : ITransactionService
    {
        ...
        public TransactionService(
            ...
            IInvestorService _inv,
            INotificationService notificationService,
            IHubContext<NotificationHub> hubContext,
            IInvestorRepo investorRepo,
            IFundRepo fundRepo,
            IFundRepo investmentRepo,
            ITransactionRepo fundTransactionRepo,
            HubBroadCastService broadcastService
        )
        {
            _repo = repo;
            _logger = logger;
        ...
        public async Task<string> HandleTransactionAsync(TransactionRequestDTO dto)
        {
            ...
            try
            {
                ...
                var portfolio = await _repo.GetPortfolioByInvestorAndStockAsync(
                ...
                if (dto.TransactionType == TransactionType.BUY)
                {
                    ...
                    if (portfolio == null)
                    {
                        ...
                        _logger.LogInformation(
                            $" New stock added to portfolio for Investor {dto.PublicInvestorId}. Bought {dto.Quantity} shares of {stock.StockSymbol} at ₹{cur_price} each."
                        );
                    }
                    else
                    {
                        ...
                        _logger.LogInformation(
                            $"🟢 Investor {dto.PublicInvestorId} bought {dto.Quantity} more shares of {stock.StockSymbol}. New total quantity: {portfolio.Quantity}."
                        );
                    }

                    // ✅ Deduct funds from investor
                    investor.InvestorDetail.Fund -= tot_price;
                    await _repo.UpdateInvestorFundAsync(
                        investor.PublicInvestorId,
                        investor.InvestorDetail.Fund
                    );
                    _logger.LogInformation(
                        $"💸 Deducted ₹{tot_price} from Investor {dto.PublicInvestorId}. Remaining Fund: ₹{investor.InvestorDetail.Fund:F2}"
                    );
                    ...
                    _logger.LogInformation(
                        $"📈 Investor {dto.PublicInvestorId} bought {dto.Quantity} shares of {stock.StockSymbol}. Price increased by {priceIncreasePercent:P}. New price: ₹{stock.CurrentMarketPrice:F2}"
                    );
                }
                // =====================================================================
                // 🔹 SELL LOGIC
                ...
                else if (dto.TransactionType == TransactionType.SELL)
                {
                    ...
                    _logger.LogInformation(
                        $"💰 Added ₹{tot_price} to Investor {dto.PublicInvestorId} after selling. New Fund Balance: ₹{investor.InvestorDetail.Fund:F2}"
                    );
                    ...
                    _logger.LogInformation(
                        $"🔴 Investor {dto.PublicInvestorId} sold {dto.Quantity} shares of {stock.StockSymbol}. Price dropped by {priceDropPercent:P}. New price: ₹{stock.CurrentMarketPrice:F2}"
                    );
                    ...
                    if (portfolio.Quantity <= 0)
                    {
                        ...
                        _logger.LogInformation(
                            $"🗑️ All shares of {stock.StockSymbol} sold. Portfolio entry removed for Investor {dto.PublicInvestorId}."
                        );
                ...
                if (portfolio != null)
                {
                    ...
                    portfolio.ProfitLoss =
                        (stock.CurrentMarketPrice - portfolio.AvgBuyPrice) * portfolio.Quantity;

                    if (portfolio.ProfitLoss > 0)
                        _logger.LogInformation(
                            $"✅ Portfolio Profit: +₹{portfolio.ProfitLoss:F2} for {stock.StockSymbol}"
                        );
                    else if (portfolio.ProfitLoss < 0)
                        _logger.LogWarning(
                            $"⚠️ Portfolio Loss: ₹{portfolio.ProfitLoss:F2} for {stock.StockSymbol}"
                        );
                    else
                        _logger.LogInformation(
                            $"ℹ️ No profit/loss for {stock.StockSymbol} (breakeven)."
                        );
                ...
                _logger.LogInformation(
                    $"✅ Transaction completed successfully for Investor {dto.PublicInvestorId} on {stock.StockSymbol}."
                );

                // 🔹 Broadcast stock update via SignalR
                await _broadcastService.BroadcastStockAsync(
                    new BroadCastStockUpdDTO
                    {
                ...
                _logger.LogInformation(
                    $"📢 Stock Update Broadcasted to [Stock-{stock.StockId}] Group."
                );
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(
                    $"❌ Transaction failed for Investor {dto.PublicInvestorId}: {ex.Message}"
                );
        ...
        public async Task<
            IEnumerable<StockTransactionsResponseDTO>
        > GetStockTransactionsByInvestorAsync(Guid investorId)
        {
            var transactions = await _repo.GetTransactionsByInvestorIdAsync(investorId);

            var response = transactions
                .Select(t => new StockTransactionsResponseDTO
                {
            ...
            _logger.LogInformation(
                "✅ Mapped {Count} stock transactions for InvestorId: {InvestorId}",
                response.Count,
                investorId
            );
            return response;
        ...
        {
            _logger.LogInformation(
                "🎯 [START] Investor Fund Purchase initiated for InvestorId: {InvestorId}, FundId: {FundId}, Amount: {Amount}",
                dto.PublicInvestorId,
                dto.FundId,
            ...
            try
            {
                ...
                {
                    _logger.LogWarning(
                        "⚠️ Investor not approved: {InvestorId}",
                        dto.PublicInvestorId
                    );
                ...
                {
                    _logger.LogError(
                        "🚫 Investor details missing for {InvestorId}",
                        dto.PublicInvestorId
                    );
                ...
                if (investor.InvestorDetail.Fund < dto.TransactionAmount)
                {
                    ...
                    throw new InvalidOperationException(
                        "Insufficient wallet balance to make this purchase."
                    );
                }
                ...
                {
                    _logger.LogWarning(
                        "⚠️ Fund not approved for investment: {FundName}",
                        fund.FundName
                    );
                    throw new InvalidOperationException(
                        "Mutual fund is not approved for investment."
                    );
                }
                ...
                {
                    _logger.LogWarning(
                        "⚠️ Investment below minimum requirement. Required: {Min}, Given: {Amount}",
                        fund.MinInvestmentAmount,
                        dto.TransactionAmount
                    );
                    throw new ArgumentException(
                        $"Minimum investment amount is {fund.MinInvestmentAmount}."
                    );
                }
                ...
                {
                    _logger.LogError(
                        "📉 Invalid NAV for Fund: {FundName}, NAV: {NAV}",
                        fund.FundName,
                        fund.NAV
                    );
                ...
                _logger.LogInformation(
                    "📊 Computed Units: {Units} for Amount: {Amount} at NAV: {NAV}",
                    unitsPurchased,
                    dto.TransactionAmount,
                    nav
                );
                ...
                var existingInvestment = await _investmentRepo.GetInvestmentByInvestorAndFundAsync(
                    dto.PublicInvestorId,
                    dto.FundId
                );
                ...
                {
                    _logger.LogInformation(
                        "🆕 New investment. Creating entry for investor {InvestorId} in fund {FundName}.",
                        dto.PublicInvestorId,
                        fund.FundName
                    );
                }
                ...
                {
                    _logger.LogInformation(
                        "🔁 Existing investment found (InvestId: {InvestId}). Updating totals.",
                ...
                _logger.LogInformation(
                    "🧾 Transaction object prepared for InvestorId: {InvestorId}, FundId: {FundId}, Amount: {Amount}",
                    dto.PublicInvestorId,
                    dto.FundId,
                    dto.TransactionAmount
                );
                ...
                var success = await _fundTransactionRepo.PerformFundPurchaseAsync(
                    investor,
                    fund,
                    existingInvestment,
                    transaction
                ...
                {
                    _logger.LogInformation(
                        "✅ Investment transaction completed successfully for InvestorId: {InvestorId}, FundId: {FundId}. Amount: {Amount}",
                        dto.PublicInvestorId,
                        dto.FundId,
                        dto.TransactionAmount
                    );
                    ...
                    await _hubContext
                        .Clients.Group($"Fund-{fund.FundId}")
                ...
                {
                    _logger.LogWarning(
                        "⚠️ Transaction repo returned false for InvestorId: {InvestorId}, FundId: {FundId}.",
                        dto.PublicInvestorId,
                        dto.FundId
                    );
                }
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "❌ Exception during Fund Purchase for InvestorId: {InvestorId}, FundId: {FundId}",
                    dto.PublicInvestorId,
                    dto.FundId
                );
            ...
            {
                _logger.LogInformation(
                    "🏁 [END] Fund Purchase operation for InvestorId: {InvestorId}, FundId: {FundId}",
                    dto.PublicInvestorId,
                    dto.FundId
                );
            }
        ...
        public async Task<
            IEnumerable<FundTransactionResponseDTO>
        > GetFundTransactionsByInvestorAsync(Guid investorId)
        {
            var transactions = await _repo.FetchFundTransactionsByInvestorIdAsync(investorId);

            var response = transactions
                .Select(t => new FundTransactionResponseDTO
                {
                    TransactionId = t.TransactionId,
            ...
            _logger.LogInformation(
                "✅ Mapped {Count} fund transactions for InvestorId: {InvestorId}",
                response.Count,
                investorId
            );
            return response;
