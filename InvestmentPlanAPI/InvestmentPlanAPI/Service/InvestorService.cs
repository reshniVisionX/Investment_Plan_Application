...
namespace InvestmentPlanAPI.Service
{
    public class InvestorService : IInvestorService
    {
        ...
        public InvestorService(IInvestorRepo repo, IWebHostEnvironment env, IConfiguration configuration)
        {
                _repo = repo;
                _env = env;
            ...
            public async Task<Investor?> LoginInvestor(InvestorLoginDTO dto)
            {
                ...
                if (found == null) throw new Exception("Invalid credentials.");
                return found;
            }
        ...
        public async Task<bool> SignUpInvestor(InvestorsSignUpDTO dto)
        {
            ...
            var existingInvestor = await _repo.FindIfDetailsExist(dto.AadhaarNo, dto.PanNo, dto.Mobile);
           

            if (existingInvestor != null)
            {
                if (existingInvestor.AadhaarNo == dto.AadhaarNo)
                    throw new Exception("This Aadhaar number is already registered with another investor.");
                if (existingInvestor.PanNo == dto.PanNo)
                    throw new Exception("This PAN number is already registered with another investor.");
                if(existingInvestor.Mobile == dto.Mobile)
                    throw new Exception("This mobile number is already registered with another investor.");
            }
                    ...
                    var placeholders = new Dictionary<string, string>
        {
            ...
            { "Message", "We’re excited to have you on board and look forward to supporting your investment journey." }
        };
        ...
        public async Task<bool> UpdateInvestor(InvestorsUpdDTO dto)
            ...
            var investor = await _repo.GetInvestorById(dto.PublicInvestorId);
                ...
                if (invDetails == null) throw new Exception("Investor details not found.");

                if (!string.IsNullOrEmpty(dto.InvestorName)) investor.InvestorName = dto.InvestorName;
            if (!string.IsNullOrEmpty(dto.Email)) investor.Email = dto.Email;
            if (!string.IsNullOrEmpty(dto.Password)) investor.Password = dto.Password;
                if (!string.IsNullOrEmpty(dto.Mobile)) invDetails.Mobile = dto.Mobile;
                if (dto.Age.HasValue) invDetails.Age = dto.Age.Value;
                if (dto.Fund.HasValue) invDetails.Fund = dto.Fund.Value;
                if (!string.IsNullOrEmpty(dto.NomineeName)) invDetails.NomineeName = dto.NomineeName;
                if (!string.IsNullOrEmpty(dto.NomineeEmail)) invDetails.NomineeEmail = dto.NomineeEmail;
                if (!string.IsNullOrEmpty(dto.NomineeRelation)) invDetails.NomineeRelation = dto.NomineeRelation;
        ...
        public async Task<(bool success, string message)> CheckDuplicateInvestorAsync(InvestorDuplicateCheckDTO dto)
        {
            var message = await _repo.CheckInvestorDetailsExistAsync(dto.AadhaarNo, dto.PanNo, dto.Mobile, dto.Email);
